...
valid_indices = np.where((target_data >= 14.0) & (target_data <= 18))[0]

# Randomly select  data points within the range for training
selected_indices = np.random.choice(valid_indices,2000, replace=False)
...
test_indices = np.where((target_data < 14) |  (target_data > 18.0))[0]

...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 20 11:48:13 2024
...
predictor_data = np.genfromtxt('/home/tarak/hashmi/Ising_Model/binary_dataset.txt', delimiter=',')
# Load target data from the new directory (modify the file path)
target_data = np.genfromtxt('/home/tarak/hashmi/Ising_Model/Energy.txt', delimiter=',')

# Create an array of indices within the range (3 to 5)
valid_indices = np.where((target_data >= -5.0) & (target_data <= 5))[0]

# Randomly select  data points within the range for training
selected_indices = np.random.choice(valid_indices,200000, replace=False)
...
test_indices = np.where((target_data < -5) | (target_data > 5.0))[0]
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(20, activation='relu'),      # Fully connected hidden layer with ReLU activation
...
epochs = 172
history = model.fit(X_train, y_train, epochs=epochs, batch_size=42, verbose=1)
...
# Add or subtract a certain amount to extend the diagonal line
extension_amount = 2  # You can adjust this value as needed

min_val_extended = min_val - extension_amount
max_val_extended = max_val + extension_amount

# Plot the extended diagonal line
plt.plot([min_val_extended, max_val_extended], [min_val_extended, max_val_extended], 'k-', linewidth=4)
...
plt.grid(False)
plt.xticks([-20, -10,0, 10, 20], fontsize=18, weight='bold')
plt.yticks([-20, -10, 0,10, 20], fontsize=18, weight='bold')
